...
class DynamicForm extends TkLocalizerMixin(PolymerElement) {
    ...
    _shouldDisplay(layoutItem, readOnly, compact) {
        return this.dataProvider ?
            (!readOnly && !compact
                ...
                || (this._rawValue(layoutItem) && !this._isMeasureField(layoutItem) && !(this._isMedicationField(layoutItem) || this._isTokenField(layoutItem)))):
            ( !readOnly && !compact
    ...
    _valueContainersChanged(event) {
        ...
        const layoutItem = this.layoutItemPerName[event.target.id];
        if (layoutItem) {
            this._isTokenField(layoutItem) ? this.dataProvider.setValueContainers(layoutItem.name, change.value) : this._isMedicationField(layoutItem) ? this.dataProvider.setValueContainers(layoutItem.name, change.medications, change.saveNow||false) : null;
        }
    }
