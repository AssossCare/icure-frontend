...
import './dialogs/flatrate-invoice/ht-msg-flatrate-invoice-accepted';
import './dialogs/flatrate-invoice/ht-msg-flatrate-invoice-archived';
import './dialogs/flatrate-invoice/ht-msg-flatrate-invoice-pending';
import './dialogs/flatrate-invoice/ht-msg-flatrate-invoice-rejected';
import './dialogs/flatrate-invoice/ht-msg-flatrate-invoice-toBeCorrected';
...
class htMsgElectronicFlatrateInvoice extends TkLocalizerMixin(PolymerElement) {
    ...
    static get is() {
        return 'ht-msg-electronic-flatrate-invoice';
    }
    ...
    _closeAllPanel(){
        this.shadowRoot.querySelector('#htMsgFlatrateInvoiceInvoiceDetail') ? this.shadowRoot.querySelector('#htMsgFlatrateInvoiceInvoiceDetail')._closeDetailPanel() : null
        this.shadowRoot.querySelector('#htMsgFlatrateInvoiceBatchDetail') ? this.shadowRoot.querySelector('#htMsgFlatrateInvoiceBatchDetail')._closeDetailPanel(): null
    }
        ...
        this.api.invoice().listToInsurancesUnsent(this.user.id).then(invoicesUnsent => {
            ...
            prom = prom.then(listOfEfact =>
                ...
                return ({
                    ...
                    normalizedSearchTerms: _.map(_.uniq(_.compact(_.flatten(_.concat([_.get(efact, _.trim('patient.lastName'), ""), _.get(efact, _.trim('patient.firstName'), ""), _.trim(_.get(efact, 'invoice.invoiceReference', "")).toString(), _.trim(_.get(efact, 'insurance.code', "")).toString(), _.trim(_.get(efact, 'patient.ssin', "")).toString(), _.trim(_.get(efact, 'invoice.invoiceDate', '')).toString(), _.get(efact, 'invoice.invoiceDate', null) ? _.trim(_.get(efact, 'invoice.invoiceDate', '')).format('DD/MM/YYYY') : ''])))), i =>  _.trim(i).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "")).join(" ")
                })
            })).then(invoices => {
                                        ...
                                        .then(a => {
                                            ...
                                            return (zone200 && zone300) ?
                                                ({
                                                    ...
                                                    normalizedSearchTerms: _.map(_.uniq(_.compact(_.flatten(_.concat([_.trim(_.get(msg, 'metas.ioFederationCode', "")), _.trim(_.get(this.hcp, 'firstName', "")), _.trim(_.get(this.hcp, 'lastName', "")), _.trim(zone300.zones && zone300.zones.find(z => z.zone === "301") ? zone300.zones.find(z => z.zone === "301").value : ""), _.trim(zone300.zones &&zone300.zones.find(z => z.zone === "300") ? zone300.zones.find(z => z.zone === "300").value : ""), _.trim(zone300.zones && zone300.zones.find(z => z.zone === "302") ? zone300.zones.find(z => z.zone === "302").value : ""), _.trim(zone300.zones && zone300.zones.find(z => z.zone === "302") ? moment(zone300.zones.find(z => z.zone === "302").value).format("DD/MM/YYYY") : ""), _.trim(msg.metas && msg.metas.paymentReferenceAccount1 ? msg.metas.paymentReferenceAccount1 : "")])))), i =>  _.trim(i).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "")).join(" ")
                                                }) : {}
                                        }) : Promise.resolve({
                                        message: msg,
                                        messageInfo: {
...
customElements.define(htMsgElectronicFlatrateInvoice.is, htMsgElectronicFlatrateInvoice);
