  ...
  static get is() {
        ...
        this.api.pdfReport(mustache.render(pdfTemplate, pdfData))
            .then(({pdf:pdfFileContent, printed:printed}) =>{
                ...
                return this.api.message().newInstance(this.user)
                    ...
                    .then(createdDocument => this.api.encryptDecrypt("encrypt", this.user, createdDocument, pdfFileContent).then(encryptedFileContent => ({createdDocument, encryptedFileContent })))
            ...
            if(singleMessage && singleMessage[0] && singleMessage[0].id && singleMessage[0].attachmentId && singleMessage[0].secretForeignKeys) {
                this.api.document().getDocumentAttachment(singleMessage[0].id, singleMessage[0].attachmentId, singleMessage[0].secretForeignKeys).then(attachmentResponse => {
                    if(attachmentResponse) {
                        this.api.encryptDecrypt("decrypt", this.user, _.head(singleMessage), attachmentResponse).then(decryptedFileContent =>{
    ...
    _downloadParentOaArchive(e) {
        ...
        return !messageId ? null : this.api.document().findByMessage(this.user.healthcarePartyId, batchMessage)
            ...
            .then(({doc,attachment})=> attachment ? this.api.encryptDecrypt("decrypt", this.user, doc, attachment).then(decryptedContent=>{return {doc:doc,decryptedContent:decryptedContent }}) : prom)
                ...
                return Promise.all(this.flatRateInvoicingDataObject.generatedFiles.flatFiles.map( singleFileObject =>  {
                    return this.api.message().newInstance(this.user)
                        ...
                        .then(createdDocument => {
                            ...
                            return this.api.encryptDecrypt("encrypt", this.user, createdDocument, this.api.crypto().utils.ua2ArrayBuffer(this.api.crypto().utils.text2ua(singleFileObject.fileContent))).then(encryptedFileContent => ({createdDocument, encryptedFileContent}))
                        })
                ...
                return Promise.all(this.flatRateInvoicingDataObject.generatedFiles.pdfs.map( singlePdfFileObject =>  {
                    return this.api.message().newInstance(this.user)
                        ...
                        .then(createdDocument => {
                            ...
                            return this.api.encryptDecrypt("encrypt", this.user, createdDocument, singlePdfFileObject.fileContent).then(encryptedFileContent => ({createdDocument, encryptedFileContent}))
                        })
            ...
            .then(batchZipFileBlob=>{
                ...
                return this.api.message().newInstance(this.user)
                    ...
                    .then(createdDocument => {
                        ...
                        return this.api.encryptDecrypt("encrypt", this.user, createdDocument, this.flatRateInvoicingDataObject.finalArchive.batchZipFileBlob).then(encryptedFileContent => ({createdDocument, encryptedFileContent}))
                    })
