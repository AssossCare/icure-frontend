...
class HtMsgList extends TkLocalizerMixin(PolymerElement) {
    ...
    _decryptAndResolvePatientsMetas(givenMessages) {
        ...
        _.map(givenMessages, singleMessage => {
            prom = prom.then(promisesCarrier => !_.trim(_.get(singleMessage,"metas.annexesInfos")) ?
                ...
                this.api.encryptDecrypt("decrypt", this.user, singleMessage, this.api.crypto().utils.text2ua(Base64.decode(_.trim(_.get(singleMessage,"metas.annexesInfos")))) )
                    ...
                    .catch(e=>{console.log("ERROR with encryptDecrypt: ",e); return Promise.resolve(_.concat(promisesCarrier, singleMessage))})
            )
        })
    ...
    _deleteAssignment(givenMessage, additionalParameters) {
        ...
        return this.api.contact().getContactWithUser(_.get(this,"user",{}), _.trim(contactIdToDelete))
            ...
            .then(() => this.api.encryptDecrypt( "encrypt", _.get(this,"user",{}), givenMessage, this.api.crypto().utils.ua2ArrayBuffer(this.api.crypto().utils.text2ua(JSON.stringify(updatedAnnexesInfos))))
    ...
    _saveAssignment(givenMessage, additionalParameters) {
        ...
        return (!documentId || !patientId || !_.size(documentToAssign)) ?
            promResolve : this.api.getDocumentTypes(this.resources, this.language)
                ...
                .then(updatedAnnexesInfos => this.api.encryptDecrypt( "encrypt", _.get(this,"user",{}), givenMessage, this.api.crypto().utils.ua2ArrayBuffer(this.api.crypto().utils.text2ua(JSON.stringify(updatedAnnexesInfos.map(ai=>_.omit(ai,"patientData"))))))
                    .then(encryptedContent => _.merge({},givenMessage, {metas:{annexesInfos:Base64.encode(String.fromCharCode.apply(null, new Uint8Array(encryptedContent)))}}))
