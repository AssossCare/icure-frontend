    ...
    return html`
        <style include="dialog-style scrollbar-style buttons-style paper-tabs-style notification-style">
            ...
            .notification-msg-alert{
                background: var(--app-primary-color-dark)!important;
            }
            ...
            <div class="rnConsultDialog">
                ...
                <div class="rn-container">
                    ...
                    <div class="rn-btn-right">
                        ...
                        <paper-button class="button button--save" autofocus on-tap="_consultHistory"><iron-icon icon="vaadin:search" class="mr5 icon-button" ></iron-icon> [[localize('rn-history','History',language)]]</paper-button>
                    </div>
                </div>
            </div>
        </paper-dialog>
        ...
        <paper-item id="rnConsult-changed-notification" class="notification-container dark">
            <iron-icon class="notification-icn" src="[[_rnConsultPicture()]]"></iron-icon>
            <div class="notification-msg notification-msg-alert">
        ...
        <paper-item id="rnConsult-no-change-notification" class="notification-container dark">
            <iron-icon class="notification-icn" src="[[_rnConsultPicture()]]"></iron-icon>
            <div class="notification-msg notification-msg-alert">
                <h4>[[localize('rn-consult-not', 'Rn consult changed', language)]]</h4>
                <p>[[localize('rn-consult-niss-no-changed', "Niss number not changed", language)]]</p>
            </div>
            <paper-button name="closePostit" class="notification-btn" on-tap="_closeRnConsultNoChangeNotification">
                [[localize('clo','Close',language)]]
            </paper-button>
        </paper-item>
...
`;
  ...
  _selectedGenderTypeChanged(){
    ...
    _consultHistory(){
        this.api.fhc().RnConsult().historyUsingGET(_.get(this, 'api.keystoreId', null), _.get(this, 'api.tokenId', null), _.get(this, 'api.credentials.ehpassword', null), _.get(this.patient, 'ssin', null)).then(resp => {
            ...
            if(!_.isEmpty(resp)){
                if(_.get(resp, 'ssin.value', null) && !_.get(resp, 'ssin.replaces', null) && _.get(resp, 'ssin.canceled', null) !== true){
                    this._rnConsultNoChangeNotification()
    ...
    _rnConsultNoChangeNotification(){
        this.$["rnConsult-no-change-notification"].classList.add('notification')
        setTimeout(() => {
            this._closeRnConsultNoChangeNotification()
        }, 600000);
    }
    ...
    _closeRnConsultNoChangeNotification(){
        this.$["rnConsult-no-change-notification"].classList.remove('notification')
    }
