    ...
    mergeCalendarItems(apps, mikronoTypeIdByTopazTypeId, userList){
        ...
        userList.forEach(user => {
            ...
            //items = items.filter(item => item.patientId && parseInt(_.get(item, "startTime", 0)) && parseInt(_.get(item, "endTime", 0))).map(item => ({
                ...
                startTime: (parseInt(_.get(item, "startTime", 0)) ? moment(_.trim(_.get(item, "startTime", "")), "YYYYMMDDHHmmss").utc().format("YYYY-MM-DDTHH:mm:ss") + "Z" : null),
                endTime: (parseInt(_.get(item, "startTime", 0)) ? moment(_.trim(_.get(item, "endTime", "")), "YYYYMMDDHHmmss").utc().format("YYYY-MM-DDTHH:mm:ss") + "Z" : null),
