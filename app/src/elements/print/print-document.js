...
`;
  ...
  _getDocument(user, documentId) {
      ...
      return !_.trim(documentId) ? promResolve : this.api.document().getDocument(documentId)
          ...
          .then(([document,enckeys,canHandle]) => !canHandle ? Promise.resolve([document,enckeys]) : this.api.beresultimport().getInfos(_.trim(_.get(document,"id","")), null, (enckeys||[]).join(','), true).then(docInfo=>([_.merge({},document,{docInfo:this._prettifyDocInfo(_.head(docInfo))}),enckeys])).catch(() => Promise.resolve([document,enckeys])))
          .then(([document,enckeys]) => (this._heicMimeType(document) ? this.api.document().getAttachmentAs(_.trim(_.get(document,"id","")), _.trim(_.get(document,"attachmentId","")), "application/octet-stream", (enckeys||[]).join(',')) : this.api.document().getDocumentAttachment(_.trim(_.get(document,"id","")), _.trim(_.get(document,"attachmentId","")), (enckeys||[]).join(',')) ).then(attachmentContent => _.merge({}, document, {attachment: {content:attachmentContent, downloadUrl:this.api.document().getAttachmentUrl(_.trim(_.get(document,"id","")), _.trim(_.get(document,"attachmentId","")), enckeys, _.get(this,"api.sessionId"))}})).catch(()=>Promise.resolve(document)))          .then(document => {
