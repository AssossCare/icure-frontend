...
`;
  ...
  ready() {
    ...
    loadMikronoIframe(proxy,electron) {
        this.set("showAgenda", true);
        this.set("mikronoUrl", `${proxy}/iCureShortcut.jsp?id=${this.user.id}&locale=${encodeURIComponent(this.language)}&electronUrl=${encodeURIComponent(electron)}`);
    }
}
