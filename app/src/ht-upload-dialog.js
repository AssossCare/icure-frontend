                    ...
                    <div id="form">
                        <template is="dom-if" if="[[_contactOptionsDisabled]]">
                            <div class="input-form">
                                <vaadin-text-field id="TitleTextField" label="[[localize('docTitle','Titre',language)]]"
                                                   value="{{title}}"
                                                   disabled="[[_patientDisabled]]"></vaadin-text-field>
                            </div>
                        </template>
                        <div class="input-form">
                            <vaadin-combo-box id="hcpCombo" filter={{filterHcp}} filtered-items="[[listHcp]]"
                                              on-filter-changed="_filterHcpChanged" on-value-changed="_hcpChanged"
                                              item-label-path="name"
                                              item-value-path="id"
                                              label="[[localize('persphysicianRole','Recipient',language)]]"
                                              value="{{hcpId}}"
                                              disabled="[[_patientDisabled]]">
                            </vaadin-combo-box>
                        </div>
                        <div class="input-form">
                            <vaadin-combo-box id="patCombo" filter="{{filterPatient}}"
                                              on-filter-changed="_filterPatChanged"
                                              label="[[localize('pat-name','Nom du patient',language)]]"
                                              filtered-items="[[listPat]]" item-label-path="name" item-value-path="id"
                                              value="{{patientId}}"
                                              disabled="[[_patientDisabled]]"></vaadin-combo-box>
                        ...
                        <template is="dom-if" if="[[_contactOptionsVisible]]">
                            <div class="input-form">
                                <vaadin-combo-box id="optionsCombo" filtered-items="[[contactOptions]]"
                                                  item-label-path="label" item-value-path="id"
                                                  label="[[localize(contact_type,'Type de contact',language)]]"
                                                  value="{{contactOption}}"
                                                  disabled="[[_contactOptionsDisabled]]">
                                </vaadin-combo-box>
                                <vaadin-date-picker id="datePicker"
                                                    label="[[localize('doc_date','Date du document',language)]]"
                                                    value="{{_documentDate}}" i18n="[[i18n]]"
                                                    disabled="[[_documentDateDisabled]]"></vaadin-date-picker>
                            </div>
                        </template>
                    </div>
...
<!--                            </template>-->
                    ...
                    <div class="input-form">
                        <vaadin-combo-box id="scanner-list" filtered-items="[[scannerList]]" item-label-path="label"
                                          item-value-path="id"
                                          label="[[localize('scanner','Scanner',language)]]"
                                          value="{{selectedScanner}}"
                                          disabled="[[!_canConfigureScan(isScanning,hasElectron,scannerList)]]">
                        </vaadin-combo-box>
                        <div class="scan-option">
                            <paper-checkbox checked={{scannerColor}}
                                            disabled="[[!_canConfigureScan(isScanning,hasElectron,scannerList)]]">
                                [[localize('color','Color',language)]]
                            </paper-checkbox>
                        </div>
                        <div class="scan-option">
                            <paper-checkbox checked={{scannerDuplex}}
                                            disabled="[[!_canConfigureScan(isScanning,hasElectron,scannerList)]]">
                                [[localize('duplex','Duplex',language)]]
                            </paper-checkbox>
                        </div>
                    </div>
                    ...
                    <div id="import-buttons">
                        <div id="upload">
                            <vaadin-upload id="vaadin-upload" no-auto files="{{files}}"
                                           accept="image/jpeg,application/pdf"
                                           form-data-name="attachment"></vaadin-upload>
                        </div>
                        <paper-button class="button button--other" role="button" on-tap="_launchScan"
                                      disabled="[[!_canConfigureScan(isScanning,hasElectron,scannerList)]]">
                ...
                <div id="info-panel" class="panel">
                    <div id="form-header">
                        ...
                        <div class="input-form" id="CommentTextForm">
                            <vaadin-text-area class="textarea-style" id="CommentTextArea"
                                              label="[[localize('com','Comments',language)]]"
                                              value="{{selectedDoc.comment}}"></vaadin-text-area>
                        </div>
                    </div>
                    <div id="visual-panel">
            ...
            <div class="buttons">
                ...
                <paper-button class="button button--save" role="button" autofocus on-tap="_saveDocuments"
        ...
        <paper-dialog id="error-message-box" class="modalDialog" no-cancel-on-outside-click no-cancel-on-esc-key
                      always-on-top>
            ...
            <div class="buttons">
                <paper-button class="button button--other" role="button" dialog-dismiss>
...
`;
    ...
    ready() {
        ...
        this.set("contactOptions", [
            ...
            {
                id: "new",
                label: this.localize("new_sync_contact", "Nouveau contact synchronisé", this.language)
            }])
    ...
    _hasElectronChanged() {
        ...
        Promise.all([fetch(`${_.get(this, "api.electronHost", "http://127.0.0.1:16042")}/scanning`, {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({
                request: "list"
            })
        }), fetch(`${_.get(this, "api.electronHost", "http://127.0.0.1:16042")}/getPrinterSetting`, {
            method: "POST",
            headers: {
    ...
    _launchScan() {
        ...
        fetch(`${_.get(this, "api.electronHost", "http://127.0.0.1:16042")}/scanning`, {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({
                request: "scan",
